...
static int kernel_context_debug_logs = 0;
...
void InitializePCB(PCB* pcb) {
    ...
    pcb->kernel_stack_copied = 0;
    pcb->kernel_clone_source = NULL;
    pcb->waiting_on_lock_id = -1;
}
...
void TerminateProcess(PCB* pcb, int exit_status) {
    ...
    int is_init = (pcb == kernel_state.init_process);
    ...
    if (is_init) {
        TracePrintf(0, "Init terminated (status %d). Halting kernel as required.\n", exit_status);
...
void Dispatch(PCB* next_process) {
    ...
    TracePrintf(1, "Dispatch: next PID %d state=%d userPC=%p userSP=%p kctx=%d kstack=%d\n",
                next_process->pid,
                next_process->state,
                next_process->user_context.pc,
                next_process->user_context.sp,
                next_process->kernel_context_valid,
                next_process->kernel_stack_copied);
...
KernelContext* KCSwitch(KernelContext* kc_in, void* curr_pcb_p, void* next_pcb_p) {
    ...
    if (curr_pcb != NULL && kc_in != NULL) {
        ...
        if (kernel_context_debug_logs < 8 && curr_pcb->kernel_stack_frames != NULL) {
            TracePrintf(0, "DEBUG: save ctx PID %d kstack_cs=0x%x frames=[%d %d]\n",
                        curr_pcb->pid,
                        curr_pcb->kernel_context.kstack_cs,
                        curr_pcb->kernel_stack_frames[0],
                        curr_pcb->kernel_stack_frames[1]);
            kernel_context_debug_logs++;
        }
    }

    if (!next_pcb->kernel_context_valid) {
        PCB* clone_src = next_pcb->kernel_clone_source;
        const KernelContext* ctx_src = NULL;

        if (clone_src != NULL && clone_src->kernel_context_valid) {
            ctx_src = &clone_src->kernel_context;
        } else if (kc_in != NULL) {
            ctx_src = kc_in;
        }

        const char* ctx_source = "none";
        if (clone_src != NULL && clone_src->kernel_context_valid) {
            ctx_src = &clone_src->kernel_context;
            ctx_source = "clone";
        } else if (kc_in != NULL) {
        ...
        if (ctx_src == NULL) {
            TracePrintf(0, "KCSwitch: missing context source for PID %d\n", next_pcb->pid);
            return kc_in;
        }

        TracePrintf(1, "KCSwitch: initializing kernel context for PID %d\n", next_pcb->pid);
        TracePrintf(1, "KCSwitch: ctx source for PID %d = %s\n", next_pcb->pid, ctx_source);
        memcpy(&next_pcb->kernel_context, ctx_src, sizeof(KernelContext));
        next_pcb->kernel_context_valid = 1;

        PCB* stack_src = NULL;
        if (!next_pcb->kernel_stack_copied) {
            if (clone_src != NULL) {
                stack_src = clone_src;
            } else {
                stack_src = curr_pcb;
            ...
            if (stack_src != NULL) {
                CopyKernelStack(stack_src, next_pcb);
                next_pcb->kernel_stack_copied = 1;
                TracePrintf(1, "KCSwitch: PID %d copied kernel stack from PID %d\n",
                            next_pcb->pid,
                            stack_src ? stack_src->pid : -1);
            } else {
                CopyKernelStack(NULL, next_pcb);
                next_pcb->kernel_stack_copied = 1;
                TracePrintf(1, "KCSwitch: PID %d captured live kernel stack snapshot\n",
                            next_pcb->pid);
            }
        }

        next_pcb->kernel_clone_source = NULL;

        if (kernel_context_debug_logs < 16 && next_pcb->kernel_stack_frames != NULL) {
            TracePrintf(0, "DEBUG: init ctx PID %d kstack_cs=0x%x frames=[%d %d]\n",
                        next_pcb->pid,
                        next_pcb->kernel_context.kstack_cs,
                        next_pcb->kernel_stack_frames[0],
                        next_pcb->kernel_stack_frames[1]);
            kernel_context_debug_logs++;
        }
    }

    SwitchKernelStackMapping(next_pcb);
    TracePrintf(1, "KCSwitch: returning context for PID %d userPC=%p userSP=%p\n",
                next_pcb->pid,
                next_pcb->user_context.pc,
                next_pcb->user_context.sp);
