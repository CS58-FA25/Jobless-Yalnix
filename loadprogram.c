...
static int CopyToUserMemory(PCB* proc, unsigned long dest, const void* src, size_t len);
...
int LoadProgram(char *name, char *args[], PCB* proc) 
{
     ...
     */
    unsigned long argv_area = (unsigned long)cpp;
    unsigned long argv_cursor = argv_area;
    unsigned long string_cursor = (unsigned long)cp;

    int argc_value = argcount;
    if (CopyToUserMemory(proc, argv_cursor, &argc_value, sizeof(int)) == ERROR) {
        free(argbuf);
        return ERROR;
    }
    argv_cursor += sizeof(char*);

    cp2 = argbuf;
    for (int i = 0; i < argcount; i++) {
        char* arg_ptr = (char*)string_cursor;
        if (CopyToUserMemory(proc, argv_cursor, &arg_ptr, sizeof(char*)) == ERROR) {
            free(argbuf);
            return ERROR;
        }
        argv_cursor += sizeof(char*);

        int slen = (int)strlen(cp2) + 1;
        if (CopyToUserMemory(proc, string_cursor, cp2, slen) == ERROR) {
            free(argbuf);
            return ERROR;
        }
        string_cursor += slen;
    ...
    char* null_ptr = NULL;
    if (CopyToUserMemory(proc, argv_cursor, &null_ptr, sizeof(char*)) == ERROR) {
        free(argbuf);
        return ERROR;
    }
    argv_cursor += sizeof(char*);
    ...
    proc->user_context.regs[1] = (int)((char**)argv_area + 1);
    proc->user_context.regs[2] = (int)((char**)argv_area + argcount + 2);
...
static int CopyToUserMemory(PCB* proc, unsigned long dest, const void* src, size_t len) {
    ...
    while (len > 0) {
        if (dest < (unsigned long)VMEM_1_BASE || dest >= (unsigned long)VMEM_1_LIMIT) {
            TracePrintf(0, "CopyToUserMemory: destination %p outside region 1\n", (void*)dest);
            return ERROR;
        }
        int vpn = (int)((dest - VMEM_1_BASE) >> PAGESHIFT);
        if (vpn < 0 || vpn >= (VMEM_1_SIZE / PAGESIZE) || !proc->region1_ptbr[vpn].valid) {
            TracePrintf(0, "CopyToUserMemory: invalid vpn %d for dest %p\n", vpn, (void*)dest);
            return ERROR;
        }

        int offset = dest & (PAGESIZE - 1);
        size_t chunk = PAGESIZE - offset;
        if (chunk > len) {
            chunk = len;
        }
        ...
        pte_t saved_pte = kernel_state.region0_ptbr[TEMP_PAGE_VPN];
        kernel_state.region0_ptbr[TEMP_PAGE_VPN].valid = 1;
        kernel_state.region0_ptbr[TEMP_PAGE_VPN].pfn = proc->region1_ptbr[vpn].pfn;
        kernel_state.region0_ptbr[TEMP_PAGE_VPN].prot = PROT_READ | PROT_WRITE;
        WriteRegister(REG_TLB_FLUSH, (TEMP_PAGE_VPN << PAGESHIFT) + VMEM_0_BASE);

        char* temp = (char*)((TEMP_PAGE_VPN << PAGESHIFT) + VMEM_0_BASE);
        memcpy(temp + offset, src_bytes, chunk);
